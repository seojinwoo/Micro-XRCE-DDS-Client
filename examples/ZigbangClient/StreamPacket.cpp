...
const uint16_t StreamPacket::crc16_table[] = {
...
uint16_t StreamPacket::GetCalculatedCRC(std::vector<uint8_t> _Payload)
{
    uint16_t crc = 0;
...
uint16_t StreamPacket::get_uint16(const std::vector<uint8_t> &data)
{
    uint16_t value = 0;
    if (data.size() >= 2)
    {
...
void StreamPacket::SetPayload(uint8_t source, uint8_t remote, std::vector<uint8_t> one)
{
    uint16_t PayloadLength = (uint16_t)one.size();
    Frame.push_back(UXR_FRAMING_BEGIN_FLAG);
    Frame.push_back(source);
    Frame.push_back(remote);
    Frame.push_back((uint8_t)((PayloadLength >> 0) & 0xFF));
    Frame.push_back((uint8_t)((PayloadLength >> 8) & 0xFF));
    Frame.insert(Frame.end(), one.begin(), one.end());

    Payload.insert(Payload.end(), one.begin(), one.end());

    uint16_t CRC = GetCalculatedCRC(one);
    Frame.push_back((uint8_t)((CRC >> 0) & 0xFF));
    Frame.push_back((uint8_t)((CRC >> 8) & 0xFF));

    isValid = true;
}

void StreamPacket::SetFrame(std::vector<uint8_t> one)
{
    ...
    CalculatedCRC = GetCalculatedCRC(Payload);

    isValid = (ReceivedCRC == ReceivedCRC);
}

std::string StreamPacket::ShowAllInfo()
{
    ...
    oss << ShowPayload(Payload);

    return oss.str();
}

std::string StreamPacket::ShowPayload(std::vector<uint8_t> one)
{
    std::ostringstream oss;
