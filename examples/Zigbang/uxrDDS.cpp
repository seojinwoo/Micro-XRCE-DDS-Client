...
#define USE_CUSTOM_TRANSPORT

bool zigbangUXR::InitTrasport(char *ip, char *port)
{
#ifdef USE_CUSTOM_TRANSPORT
    ...
    if (!uxr_init_custom_transport(&custom_transport, NULL))
    {
...
{
#ifdef USE_CUSTOM_TRANSPORT
    uxr_init_session(&session, &custom_transport.comm, key);
#else
...
#ifdef USE_CUSTOM_TRANSPORT
    uxr_close_custom_transport(&custom_transport);
#else
